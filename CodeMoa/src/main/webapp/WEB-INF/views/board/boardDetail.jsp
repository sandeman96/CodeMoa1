...
<!-- 							style="font-size: 0.8rem; color: #939597; width: 7rem;">돌아가기</button> -->
						<c:if test="${ loginUser.id eq b.bWriter or loginUser.admin eq'Y' }">
							<button type="button" class="btn btn-warning"
								...
								<tbody>
									<tr>
										<td><span>
											...
											</c:if> <br> <span style="font-size: 1.5rem; color: black; font-weight: 500;">${ b.bTitle }</span>
											<c:if test='${ b.bStatus eq "N" }'>
												<h6 style="color: DarkSalmon;">삭제된 게시물입니다</h6>
											</c:if></td>
									...
									<tr>
										<c:if test='${ b.bStatus eq "Y" }'>
											<c:if test="${ !empty loginUser }">
												<td>
													<div>
														<button id="insertReply" class="btn bg-gradient-warning"
															type="button"
										...
										<td>
											...
											<div style="display: none;" id="afterClick">
												<button id="saveReply" class="btn bg-gradient-warning" type="button"
													style="font-size: 0.8rem; color: #939597; width: 7rem;">작성
													완료</button>
												<button id="cancelReply" class="btn bg-gradient-warning"
													type="button"
	...
	$('#saveReply').click(function() {
						...
						success : function(data) {
							...
							if (data.length > 0) {
								for ( var i in data) {
									...
									var $tdContent = $('<td id="tdContentId'+ i +'" style="width:45%;">');
									var $tr = $('<tr style="height:3rem;" id="trId'+ i +'">');
									var $move = $('<a href="mypage.me?userId='+data[i].rWriter+'">').html(data[i].nickName);
									var $writer = $('<td style="width:10%;">');
									var $date = $('<td style="width:10%;">' ).text(data[i].rDate);
									var $tdBtn = $('<td id="replyBtnArea'+i+'" style="width:20%;">');
		...
		//되살리기 버튼 클릭시
		$('#checkRevive').click(function() {
			Swal.fire({
				  title: '게시글 되살리기',
				  text: "게시글 삭제를 취소 하시겠습니까?",
				  icon: 'warning',
				  showCancelButton: true,
				  confirmButtonColor: '#3085d6',
				  cancelButtonColor: '#d33',
				  cancelButtonText: '취소',
				  confirmButtonText: '되살리기'
				}).then((result) => {
				  if (result.isConfirmed) {
						Swal.fire({
							  position: 'middle',
						...
						function saved(){
							 location.href="reviveBoard.bo?bNo=${b.bNo}&bType=${b.bType}&page=${page}&side=${side}";
						}	
