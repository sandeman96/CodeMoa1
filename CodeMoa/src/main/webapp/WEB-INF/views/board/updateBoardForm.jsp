...
<!-- Amsify Plugin -->
<link rel="stylesheet"
	href="/codemoa/resources/dist/css/amsify.suggestags.css">
					...
					<form action="updateBoard.bo" method="post">
						<div class="card card-outline card-warning">
							<div class="card-body">
								...
								<c:set var="bFirst" value="${b.bFirst}" />
								...
								<div class="col-sm-9 mb-3" style="display: inline-block;">
									...
									<input type="hidden" name="bNo" value="${b.bNo}"> <input
										...
										type="text" id="bTitle" name="bTitle" class="form-control"
										placeholder="제목을 입력하세요" value="${b.bTitle}">
								</div>
								<textarea id="summernote" name="bContent"
									placeholder="내용을 입력하세요">${b.bContent}</textarea>
							</div>
							<div class="card-body">
	...
	<script>
		$(function() {
			...
			$('#updateBoardBtn').click(
					function() {
						var text = $('#summernote').summernote('code').replace(
								/<\/?[^>]+(>|$)/g, "");
						if (($.trim($('#bTitle').val())) == '') {
							Swal.fire({
								icon : 'error',
